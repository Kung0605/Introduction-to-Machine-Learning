...
def gini(y):
    if(len(y) == 0):
        return 1
    nothing, counter = np.unique(y, return_counts = True)
    proportions = counter / len(y)
...
class DecisionTree():
    ...
    def plot_feature_importance_img(self):
...
# You can change the random seed if you want to.
    ...
    ada = AdaBoost(criterion='gini', n_estimators=18)
